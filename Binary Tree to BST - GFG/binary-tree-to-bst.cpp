...
class Solution{
    ...
    Node* generateBST(vector<int> &vals,int l,int r) {
        if (l>r) return NULL;
        if (l == r) return new Node(vals[l]);
        
        int m = (l+r)/2;
        Node* p = new Node(vals[m]);
        p->left = generateBST(vals,l,m-1);
        p->right = generateBST(vals,m+1,r);
    }
  public:
